...
Cut.Image.prototype.tile = function(inner) {
    ...
    this._repeatTicker = function() {
        ...
        // top, left
        if (btop && bleft) {
            this._slice(c++).sWidth(bleft).sX(0).sHeight(btop).sY(0).dX(left).dY(top);
        }
        // bottom, left
        if (bbottom && bleft) {
            this._slice(c++).sWidth(bleft).sX(0).sHeight(bbottom).sY(bheight + btop).dX(left).dY(top + height + btop);
        }
        // top, right
        if (btop && bright) {
            this._slice(c++).sWidth(bright).sX(bwidth + bleft).sHeight(btop).sY(0).dX(left + width + bleft).dY(top);
        }
        // bottom, right
        if (bbottom && bright) {
            this._slice(c++).sWidth(bright).sX(bwidth + bleft).sHeight(bbottom).sY(bheight + btop).dX(left + width + bleft).dY(top + height + btop);
        }
        var x = left + bleft;
        var r = width;
        while (r > 0) {
            ...
            while (b > 0) {
                ...
                b -= bheight;
                this._slice(c++).sWidth(w).sX(bleft).sHeight(h).sY(btop).dX(x).dY(y);
                if (r <= 0) {
                    // left
                    if (bleft) {
                        this._slice(c++).sWidth(bleft).sX(0).sHeight(h).sY(btop).dX(left).dY(y);
                    }
                    // right
                    if (bright) {
                        this._slice(c++).sWidth(bright).sX(bwidth + bleft).sHeight(h).sY(btop).dX(x + w).dY(y);
                    }
                }
                y += h;
            }
            // top
            if (btop) {
                this._slice(c++).sWidth(w).sX(bleft).sHeight(btop).sY(0).dX(x).dY(top);
            }
            // bottom
            if (bbottom) {
                this._slice(c++).sWidth(w).sX(bleft).sHeight(bbottom).sY(bheight + btop).dX(x).dY(y);
            }
            x += w;
        }
...
Cut.Image.prototype.stretch = function(inner) {
    ...
    this._repeatTicker = function() {
        ...
        // top, left
        if (otop && oleft) {
            this._slice(c++).sWidth(oleft).sX(0).sHeight(otop).sY(0).dX(0).dY(0);
        }
        // bottom, left
        if (obottom && oleft) {
            this._slice(c++).sWidth(oleft).sX(0).sHeight(obottom).sY(oheight - obottom).dX(0).dY(height - obottom);
        }
        // top, right
        if (otop && oright) {
            this._slice(c++).sWidth(oright).sX(owidth - oright).sHeight(otop).sY(0).dX(width - oright).dY(0);
        }
        // bottom, right
        if (obottom && oright) {
            this._slice(c++).sWidth(oright).sX(owidth - oright).sHeight(obottom).sY(oheight - obottom).dX(width - oright).dY(height - obottom);
        }
        // top
        if (otop) {
            this._slice(c++).sWidth(owidth - oleft - oright).sX(oleft).sHeight(otop).sY(0).dX(oleft).dY(0).dWidth(width - oleft - oright);
        }
        // bottom
        if (obottom) {
            this._slice(c++).sWidth(owidth - oleft - oright).sX(oleft).sHeight(obottom).sY(oheight - obottom).dX(oleft).dY(height - obottom).dWidth(width - oleft - oright);
        }
        // left
        if (oleft) {
            this._slice(c++).sWidth(oleft).sX(0).sHeight(oheight - otop - obottom).sY(otop).dX(0).dY(otop).dHeight(height - otop - obottom);
        }
        // right
        if (oright) {
            this._slice(c++).sWidth(oright).sX(owidth - oright).sHeight(oheight - otop - obottom).sY(otop).dX(width - oright).dY(otop).dHeight(height - otop - obottom);
        }
        // center
        this._slice(c++).sWidth(owidth - oleft - oright).sX(oleft).sHeight(oheight - otop - obottom).sY(otop).dX(oleft).dY(otop).dWidth(width - oleft - oright).dHeight(height - otop - obottom);
        this._cutouts.length = c;
    };
    this.tick(this._repeatTicker);
    return this;
...
Cut.Out.prototype.setup = function(def) {
    if (def) {
        this._name = def.name || this._name;
        if (typeof def.x === "number") this._x = def.x;
        if (typeof def.y === "number") this._y = def.y;
        if (typeof def.width === "number") this._width = def.width;
        if (typeof def.height === "number") this._height = def.height;
        if (typeof def.top === "number") this._top = def.top;
        if (typeof def.bottom === "number") this._bottom = def.bottom;
        if (typeof def.left === "number") this._left = def.left;
        if (typeof def.right === "number") this._right = def.right;
    }
    this._dx = 0;
    this._dy = 0;
...
Cut.drawing = function() {
    ...
    var image = new Cut.image();
    image.drawing = function(callback) {
        return image.image(drawing.drawing(callback));
    };
    return image;
};
...
function AppLoader(app, configs) {
    ...
    context.isFast = context.isFast ? true : false;
